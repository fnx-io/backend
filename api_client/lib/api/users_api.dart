...
class UsersApi {
  ...
  Future<UserEntity> getUser(int id) async {
    ...
    if(response.statusCode >= 400) {
      ...
      return 
          apiClient.deserialize(response.body, 'UserEntity') as UserEntity ;
    } else {
      return null;
    }
  }
  ...
  Future<List<UserEntity>> listUsers() async {
    ...
    if(response.statusCode >= 400) {
      ...
      return 
        (apiClient.deserialize(response.body, 'List<UserEntity>') as List).map((item) => item as UserEntity).toList();
    } else {
      return null;
    }
  }
