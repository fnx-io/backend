...
void launchApp<T>(ComponentFactory<T> componentFactory) async {
  ...
  RestClient restClient = new BrowserRestClient.root(isLocal ? "http://localhost:8085/api" : "/api");
...
FnxExceptionHandler createExceptionHandler() {
  ...
  exceptionHandler.registerErrorProcessor(ClientException, (exception, stacktrace) {
    return new FnxError("Error while trying to connect to server.", headline: "Connection error");
  });

  exceptionHandler.registerErrorProcessor(HttpException, (exception, stacktrace) {
    HttpException e = exception as HttpException;
    return new FnxError('Error code: ${e.httpStatus}, message: "${e.data}"', headline: "Server error");
  });

  /*
  exceptionHandler.registerErrorProcessor(ExpressionChangedAfterItHasBeenCheckedException, (exception, stacktrace) {
    print(exception);
    print(stacktrace);
    return null;
  });
